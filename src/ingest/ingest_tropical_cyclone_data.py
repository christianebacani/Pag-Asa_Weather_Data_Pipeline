...
def scrape_tropical_cyclone_bulletin_data(url: str) -> dict:
    '''
        The function retrieves the data containing tropical cyclone bulletin.
    '''
...
def scrape_tropical_cyclone_warning_for_shipping_data(url: str) -> dict:
    '''
        The function retrieves the data containing tropical cyclone warning for shipping.
    '''
    response = requests.get(url)
...
def scrape_forecast_storm_surge_data(url: str) -> dict:
    '''
        The function retrieves the URL containing the 
        forecasted storm surge data in PDF Format.
    '''
...
def scrape_tropical_cyclone_warning_for_agriculture_data(url: str) -> dict:
    ...
    first_instance_of_div_tag_with_row_classes = list_of_all_div_tag_with_row_classes[0]

    # Scrape the header of the tropical cyclone warning for agriculture
    div_tag_with_col_md_eight_col_sm_five_col_xs_four_text_center_class = first_instance_of_div_tag_with_row_classes.find('div', attrs={'class': 'col-md-8 col-sm-5 col-xs-4 text-center'})
    ...
    # Scrape the released datetime of tropical cyclone warning for agriculture    
    bold_tag = first_instance_of_div_tag_with_row_classes.find('b')
